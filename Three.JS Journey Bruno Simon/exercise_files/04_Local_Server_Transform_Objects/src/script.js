...
const group = new THREE.Group(); //Create a group container
group.position.y = 1;
group.scale.y = 2;
group.rotation.y = 1;
scene.add(group); //Add group container to the scene
...
cube1.position.x = -2;
group.add(cube1); //Add object to the group container
const cube2 = new THREE.Mesh(
  new THREE.BoxGeometry(1, 1, 1),
  new THREE.MeshBasicMaterial({ color: 0xffbb00 })
);
group.add(cube2); //Add object to the group container
const cube3 = new THREE.Mesh(
  new THREE.BoxGeometry(1, 1, 1),
  new THREE.MeshBasicMaterial({ color: 0xadab12 })
);
cube3.position.x = 2;
group.add(cube3); //Add object to the group container
